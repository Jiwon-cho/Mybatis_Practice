...
<body>
	...
	<form action="${path }/insertStudentData" method="post">
		이름<input type="text" name="name"><br> 전화번호<input
			type="text" name="phone"><br> 주소<input type="text"
			name="addr"><br> <input type="submit" value="저장">
	</form>

	<h4>다중값 입력 받아 수정하기</h4>
	<form action="${path }/updateStudent" method="post">
		학생번호<input type="number" name="no"><br> 이메일<input
			type="text" name="email"><br> 전화번호<input type="text"
			name="phone"><br> 주소<input type="text" name="addr"><br>
		<input type="submit" value="저장">
	</form>
	<h4>학생번호 입력받아 삭제하기</h4>
	<form action="${path }/deleteStudent" method="post">
		삭제할 학생번호 <input type="number" name="no"><br> <input
	...
	<form action="${path }/selectStudentOne">
		<input type="number" name="no"> <input type="submit"
			value="검색">
	</form>

	<h4>student테이블의 전체학생 조회하기</h4>
	<p>다수 row를 가져온다.</p>
	<a href="${path }/selectStudentAll">전체학생 조회</a>

	<h4>Map을 이용해서 데이터 가져오기</h4>
	<p>데이터보관하기 위한 vo객체를 만들지 않고 DB의 데이터 가져와 출력하기 --> Map객체이용하기 /
		key:value</p>
	<a href="${path }/selectStudentMap?no=1">학생조회</a>
